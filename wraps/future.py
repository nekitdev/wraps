...
from typing import TYPE_CHECKING, AsyncIterator, Awaitable, Callable, Generator, Optional, Type, TypeVar
...
from wraps.typing import AnyException, AsyncInspect, AsyncNullary, AsyncPredicate, AsyncUnary, Inspect, Nullary, Predicate, Unary
...
class FutureOption(Future[Option[T]]):
    ...
    def or_else_future(self, function: Unary[T, FutureOption[U]]) -> FutureOption[U]:
        return self.create(self.actual_and_then_future(function))
...
class FutureResult(Future[Result[T, E]]):
    ...
    async def actual_unwrap_error_or_raise_with_await(self, function: AsyncNullary[AnyException]) -> E:
    ...
    async def actual_map_error_or_else_await(self, default: AsyncNullary[F], function: Unary[E, F]) -> F:
    ...
    def map_error_await_or_else_await(self, default: AsyncNullary[F], function: AsyncUnary[E, F]) -> Future[F]:
    ...
    async def actual_map_error_await_or_else(self, default: Nullary[F], function: AsyncUnary[E, F]) -> F:
        return await (await self.awaitable).map_error_await_or_else(default, function)

    async def actual_map_error_await_or_else_await(self, default: AsyncNullary[F], function: AsyncUnary[E, F]) -> F:
