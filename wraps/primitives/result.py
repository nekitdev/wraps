"""Error handling with the [`Result[T, E]`][wraps.primitives.result.Result] type.

[`Result[T, E]`][wraps.primitives.result.Result] is the type used for returning and propagating errors.

It is an enum with the variants, [`Ok[T]`][wraps.primitives.result.Ok],
...
Functions return [`Result[T, E]`][wraps.primitives.result.Result] whenever errors are expected and recoverable.

For instance, our `divide` function from the [`option`][wraps.primitives.option] section:
...
class ResultProtocol(AsyncIterable[T], Iterable[T], Protocol[T, E]):  # type: ignore[misc]
    ...
    @required
    def is_ok(self) -> bool:
        """Checks if the result is [`Ok[T]`][wraps.primitives.result.Ok].
        ...
        Returns:
            Whether the result is [`Ok[T]`][wraps.primitives.result.Ok].
        """
        ...

    @required
    def is_ok_and(self, predicate: Predicate[T]) -> bool:
        """Checks if the result is [`Ok[T]`][wraps.primitives.result.Ok] and the value
        ...
        Returns:
            Whether the result is [`Ok[T]`][wraps.primitives.result.Ok] and the predicate is matched.
        """
        ...
    ...
    async def is_ok_and_await(self, predicate: AsyncPredicate[T]) -> bool:
        ...
        Returns:
            Whether the result is [`Ok[T]`][wraps.primitives.result.Ok] and
    ...
    def is_error(self) -> bool:
        """Checks if the result is [`Error[E]`][wraps.primitives.result.Error].
        ...
        Returns:
            Whether the result is [`Error[E]`][wraps.primitives.result.Error].
        """
        ...

    @required
    def is_error_and(self, predicate: Predicate[E]) -> bool:
        """Checks if the result is [`Error[E]`][wraps.primitives.result.Error] and the value
        inside of it matches the `predicate`.
        ...
        Returns:
            Whether the result is [`Error[E]`][wraps.primitives.result.Error] and the predicate is matched.
        """
        ...
    ...
    async def is_error_and_await(self, predicate: AsyncPredicate[E]) -> bool:
        ...
        Returns:
            Whether the result is [`Error[E]`][wraps.primitives.result.Error] and
            the asynchronous predicate is matched.
        """
        ...
    ...
    def expect(self, message: str) -> T:
        ...
        Raises:
            Panic: Panics with the `message` if the result is [`Error[E]`][wraps.primitives.result.Error].
    ...
    def expect_error(self, message: str) -> E:
        ...
        Raises:
            Panic: Panics with the `message` if the result is [`Ok[T]`][wraps.primitives.result.Ok].
    ...
    @required
    def unwrap(self) -> T:
        """Returns the contained [`Ok[T]`][wraps.primitives.result.Ok] value (of type `T`).

        Because this function may panic, its use is generally discouraged.

        Instead, prefer to use pattern matching and handle the [`Error[E]`][wraps.primitives.result.Error]
        case explicitly, or call [`unwrap_or`][wraps.primitives.result.ResultProtocol.unwrap_or]
        or [`unwrap_or_else`][wraps.primitives.result.ResultProtocol.unwrap_or_else].
        ...
        Raises:
            Panic: Panics if the result is [`Error[E]`][wraps.primitives.result.Error].
    ...
    def unwrap_or(self, default: T) -> T:  # type: ignore[misc]
        """Returns the contained [`Ok[T]`][wraps.primitives.result.Ok] value or the provided `default`.
    ...
    def unwrap_error(self) -> E:
        ...
        Instead, prefer to use pattern matching and handle the [`Ok[T]`][wraps.primitives.result.Ok]
        case explicitly, or call [`unwrap_error_or`][wraps.primitives.result.ResultProtocol.unwrap_error_or]
        or [`unwrap_error_or_else`][wraps.primitives.result.ResultProtocol.unwrap_error_or_else].
        ...
        Raises:
            Panic: Panics if the result is [`Ok[T]`][wraps.primitives.result.Ok].
    ...
    def unwrap_error_or(self, default: E) -> E:  # type: ignore[misc]
        """Returns the contained [`Error[E]`][wraps.primitives.result.Error] value (of type `E`)
    ...
    def raising(self: ResultProtocol[T, AnyError]) -> T:
        """Returns the contained [`Ok[T]`][wraps.primitives.result.Ok] value or raises the
        contained [`Error[AnyError]`][wraps.primitives.result.Error] value.
        ...
        Raises:
            AnyError: The contained error, if the result is [`Error[AnyError]`][wraps.primitives.result.Error].
    ...
    def ok(self) -> Option[T]:
        """Converts a [`Result[T, E]`][wraps.primitives.result.Result]
        into an [`Option[T]`][wraps.primitives.option.Option].

        Converts `self` into an [`Option[T]`][wraps.primitives.option.Option], discarding errors, if any.
    ...
    def error(self) -> Option[E]:
        """Converts a [`Result[T, E]`][wraps.primitives.result.Result]
        into an [`Option[E]`][wraps.primitives.option.Option].

        Converts `self` into an [`Option[E]`][wraps.primitives.option.Option],
    ...
    def inspect(self, function: Inspect[T]) -> Result[T, E]:
        """Inspects a possibly contained [`Ok[T]`][wraps.primitives.result.Ok] value.
    ...
    def inspect_error(self, function: Inspect[E]) -> Result[T, E]:
        """Inspects a possibly contained [`Error[E]`][wraps.primitives.result.Error] value.
    ...
    def map(self, function: Unary[T, U]) -> Result[U, E]:
        """Maps a [`Result[T, E]`][wraps.primitives.result.Result] to a [`Result[U, E]`][wraps.primitives.result.Result]
        by applying `function` to the contained [`Ok[T]`][wraps.primitives.result.Ok] value,
        leaving any [`Error[E]`][wraps.primitives.result.Error] values untouched.
    ...
    def map_error(self, function: Unary[E, F]) -> Result[T, F]:
        """Maps a [`Result[T, E]`][wraps.primitives.result.Result] to a [`Result[T, F]`][wraps.primitives.result.Result]
        by applying the `function` to the contained [`Error[E]`][wraps.primitives.result.Error] value,
        leaving any [`Ok[T]`][wraps.primitives.result.Ok] values untouched.
    ...
    async def map_await(self, function: AsyncUnary[T, U]) -> Result[U, E]:
        """Maps a [`Result[T, E]`][wraps.primitives.result.Result] to a [`Result[U, E]`][wraps.primitives.result.Result]
        by applying the asynchronous `function` to the contained [`Ok[T]`][wraps.primitives.result.Ok] value,
        leaving any [`Error[E]`][wraps.primitives.result.Error] values untouched.
    ...
    async def map_error_await(self, function: AsyncUnary[E, F]) -> Result[T, F]:
        """Maps a [`Result[T, E]`][wraps.primitives.result.Result] to a [`Result[T, F]`][wraps.primitives.result.Result]
        by applying the asynchronous `function` to the contained [`Error[E]`][wraps.primitives.result.Error]
        value, leaving any [`Ok[T]`][wraps.primitives.result.Ok] values untouched.
    ...
    def async_iter(self) -> AsyncIterator[T]:
        ...
        [`Ok[T]`][wraps.primitives.result.Ok] value.
    ...
    def async_iter_error(self) -> AsyncIterator[E]:
        """Returns an asynchronous iterator over the possibly contained
        [`Error[E]`][wraps.primitives.result.Error] value.
    ...
    def and_then(self, function: Unary[T, Result[U, E]]) -> Result[U, E]:
        """Returns the result if it is an [`Error[E]`][wraps.primitives.result.Error],
    ...
    def or_else(self, function: Unary[E, Result[T, F]]) -> Result[T, F]:
        """Returns the result if it is [`Ok[T]`][wraps.primitives.result.Ok], otherwise calls the `function`
    ...
    async def or_else_await(self, function: AsyncUnary[E, Result[T, F]]) -> Result[T, F]:
        """Returns the result if it is [`Ok[T]`][wraps.primitives.result.Ok], otherwise calls the asynchronous
    ...
    def try_flatten(self: ResultProtocol[ResultProtocol[T, E], E]) -> Result[T, E]:
        """Flattens a [`Result[Result[T, E], E]`][wraps.primitives.result.Result]
        into a [`Result[T, E]`][wraps.primitives.result.Result].

        This is equivalent to [`result.and_then(identity)`][wraps.primitives.result.ResultProtocol.and_then].
    ...
    def try_flatten_error(self: ResultProtocol[T, ResultProtocol[T, E]]) -> Result[T, E]:
        """Flattens a [`Result[T, Result[T, E]]`][wraps.primitives.result.Result]
        into a [`Result[T, E]`][wraps.primitives.result.Result].

        This is equivalent to [`result.or_else(identity)`][wraps.primitives.result.ResultProtocol.or_else].
    ...
    def flip(self) -> Result[E, T]:
        """Converts a [`Result[T, E]`][wraps.primitives.result.Result]
        into a [`Result[E, T]`][wraps.primitives.result.Result].

        [`Ok(value)`][wraps.primitives.result.Ok] and [`Error(error)`][wraps.primitives.result.Error] get swapped to
        [`Error(value)`][wraps.primitives.result.Error] and [`Ok(error)`][wraps.primitives.result.Ok] respectively.
    ...
    def into_ok_or_error(self: ResultProtocol[V, V]) -> V:
        """Returns the value contained within [`Result[V, V]`][wraps.primitives.result.Result], regardless
        of whether or not that result is [`Ok[V]`][wraps.primitives.result.Ok]
        or [`Error[V]`][wraps.primitives.result.Error].
    ...
    def into_either(self) -> Either[T, E]:
        """Converts a [`Result[T, E]`][wraps.primitives.result.Result]
        into an [`Either[T, E]`][wraps.primitives.either.Either].

        [`Ok(value)`][wraps.primitives.result.Ok] is mapped to [`Left(value)`][wraps.primitives.either.Left]
        and [`Error(error)`][wraps.primitives.result.Error] is mapped to [`Right(error)`][wraps.primitives.either.Right].
    ...
    def early(self) -> T:
        ...
        Calls to this method are to be combined with [`@early_result`][wraps.early.decorators.early_result]
...
class Ok(ResultProtocol[T, Never]):
    """[`Ok[T]`][wraps.primitives.result.Ok] variant of [`Result[T, E]`][wraps.primitives.result.Result]."""
...
class Error(ResultProtocol[Never, E]):
    """[`Error[E]`][wraps.primitives.result.Error] variant of [`Result[T, E]`][wraps.primitives.result.Result]."""
...
"""Result value, expressed as the union of [`Ok[T]`][wraps.primitives.result.Ok]
and [`Error[E]`][wraps.primitives.result.Error].
"""


def is_ok(result: Result[T, E]) -> TypeIs[Ok[T]]:
    """This is the same as [`Result.is_ok`][wraps.primitives.result.ResultProtocol.is_ok],
...
def is_error(result: Result[T, E]) -> TypeIs[Error[E]]:
    """This is the same as [`Result.is_error`][wraps.primitives.result.ResultProtocol.is_error],
    except it works as a *type guard*.
    """
...
from wraps.primitives.either import Either, Left, Right
from wraps.primitives.option import NULL, Null, Option, Some
