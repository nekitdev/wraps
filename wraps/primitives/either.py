...
The [`Either[L, R]`][wraps.primitives.either.Either] type is symmetric and treats its variants the same way,
without preference.
For representing results (values and errors), use the [`Result[T, E]`][wraps.primitives.result.Result] instead.
"""
...
class Left(EitherProtocol[L, Never]):
    """[`Left[L]`][wraps.primitives.either.Left] variant of [`Either[L, R]`][wraps.primitives.either.Either]."""
...
"""Either value, expressed as the union of [`Left[L]`][wraps.primitives.either.Left]
and [`Right[R]`][wraps.primitives.either.Right].
"""
