...
[`Option[T]`][wraps.primitives.option.Option] represents an optional value:
every [`Option[T]`][wraps.primitives.option.Option] is either [`Some[T]`][wraps.primitives.option.Some] and
contains a value, or [`Null`][wraps.primitives.option.Null], and does not.

[`Option[T]`][wraps.primitives.option.Option] types can be very common in python code,
as they have a number of uses:

- Initial values (see [`ReAwaitable[T]`][wraps.primitives.reawaitable.ReAwaitable]);
- Return values for functions not defined over their entire input range (partial functions);
- Return value for otherwise reporting simple errors, where [`Null`][wraps.primitives.option.Null]
  is returned on error;
- Optional function arguments.

[`Option[T]`][wraps.primitives.option.Option] is commonly paired with pattern matching to query
the presence of [`Some[T]`][wraps.primitives.option.Some] value (`T`) and take action,
always accounting for the [`Null`][wraps.primitives.option.Null] case:
...
Here, we know that [`Null`][wraps.primitives.option.Null] represents only one case,
that is, attempts to divide by zero. However, when we need to represent multiple errors
from one function, we might want to use [`Result[T, E]`][wraps.primitives.result.Result] instead,
as described in the [`result`][wraps.primitives.result] section.
"""
...
class OptionProtocol(AsyncIterable[T], Iterable[T], Protocol[T]):  # type: ignore[misc]
    ...
    def is_some(self) -> bool:
        """Checks if the option is [`Some[T]`][wraps.primitives.option.Some].
        ...
        Returns:
            Whether the option is [`Some[T]`][wraps.primitives.option.Some].
        """
        ...

    @required
    def is_some_and(self, predicate: Predicate[T]) -> bool:
        """Checks if the option is [`Some[T]`][wraps.primitives.option.Some] and the value
        ...
        Returns:
            Whether the option is [`Some[T]`][wraps.primitives.option.Some] and the predicate is matched.
        """
        ...
    ...
    async def is_some_and_await(self, predicate: AsyncPredicate[T]) -> bool:
        ...
        Returns:
            Whether the option is [`Some[T]`][wraps.primitives.option.Some] and
    ...
    def is_null(self) -> bool:
        """Checks if the option is [`Null`][wraps.primitives.option.Null].
        ...
        Returns:
            Whether the option is [`Null`][wraps.primitives.option.Null].
        """
        ...
    ...
    def expect(self, message: str) -> T:
        ...
        Raises:
            Panic: Panics with the `message` if the option is [`Null`][wraps.primitives.option.Null].
    ...
    def extract(self) -> Optional[T]:
        """Returns the contained [`Some[T]`][wraps.primitives.option.Some] value or [`None`][None].
    ...
    def unwrap(self) -> T:
        ...
        Instead, prefer to use pattern matching and handle the [`Null`][wraps.primitives.option.Null]
        case explicitly, or call [`unwrap_or`][wraps.primitives.option.OptionProtocol.unwrap_or]
        or [`unwrap_or_else`][wraps.primitives.option.OptionProtocol.unwrap_or_else].
        ...
        Raises:
            Panic: Panics if the option is [`Null`][wraps.primitives.option.Null].
    ...
    def unwrap_or(self, default: T) -> T:  # type: ignore[misc]
        """Returns the contained [`Some[T]`][wraps.primitives.option.Some] value or the provided `default`.
    ...
    def unwrap_or_else(self, default: Nullary[T]) -> T:
        """Returns the contained [`Some[T]`][wraps.primitives.option.Some] value or
    ...
    def inspect(self, function: Inspect[T]) -> Option[T]:
        """Inspects a possibly contained [`Option[T]`][wraps.primitives.option.Option] value.
    ...
    def map(self, function: Unary[T, U]) -> Option[U]:
        """Maps an [`Option[T]`][wraps.primitives.option.Option] to an [`Option[U]`][wraps.primitives.option.Option]
    ...
    def ok_or(self, error: E) -> Result[T, E]:
        """Transforms an [`Option[T]`][wraps.primitives.option.Option]
        into a [`Result[T, E]`][wraps.primitives.result.Result], mapping [`Some(value)`][wraps.primitives.option.Some]
        to [`Ok(value)`][wraps.primitives.result.Ok] and [`Null`][wraps.primitives.option.Null]
        to [`Error(error)`][wraps.primitives.result.Error].
    ...
    async def ok_or_else_await(self, error: AsyncNullary[E]) -> Result[T, E]:
        ...
        into a [`Result[T, E]`][wraps.primitives.result.Result], mapping [`Some(value)`][wraps.primitives.option.Some]
        to [`Ok(value)`][wraps.primitives.result.Ok] and [`Null`][wraps.primitives.option.Null]
        to [`Error(await error())`][wraps.primitives.result.Error].
    ...
    def and_then(self, function: Unary[T, Option[U]]) -> Option[U]:
        """Returns the option if it is [`Null`][wraps.primitives.option.Null],
    ...
    def filter(self, predicate: Predicate[T]) -> Option[T]:
        ...
        - [`Some(value)`][wraps.primitives.option.Some] if the contained `value` matches the predicate, and
        - [`Null`][wraps.primitives.option.Null] otherwise.
    ...
    def xor(self, option: Option[T]) -> Option[T]:
        """Returns [`Some[T]`][wraps.primitives.option.Some] if exactly one of `self` and `option`
        is [`Some[T]`][wraps.primitives.option.Option], otherwise returns [`Null`][wraps.primitives.option.Null].
    ...
    def zip(self, option: Option[U]) -> Option[Tuple[T, U]]:
        ...
        If `self` is [`Some(s)`][wraps.primitives.option.Some] and `option` is [`Some(o)`][wraps.primitives.option.Some],
        this method returns [`Some((s, o))`][wraps.primitives.option.Some]. Otherwise,
        [`Null`][wraps.primitives.option.Null] is returned.
    ...
    def zip_with(self, option: Option[U], function: Binary[T, U, V]) -> Option[V]:
        ...
        If `self` is [`Some(s)`][wraps.primitives.option.Some] and `option` is [`Some(o)`][wraps.primitives.option.Some],
        this method returns [`Some(function(s, o))`][wraps.primitives.option.Some]. Otherwise,
        [`Null`][wraps.primitives.option.Null] is returned.
    ...
    async def zip_with_await(self, option: Option[U], function: AsyncBinary[T, U, V]) -> Option[V]:
        ...
        If `self` is [`Some(s)`][wraps.primitives.option.Some] and `option` is [`Some(o)`][wraps.primitives.option.Some],
        this method returns [`Some(await function(s, o))`][wraps.primitives.option.Some]. Otherwise,
        [`Null`][wraps.primitives.option.Null] is returned.
    ...
    def unzip(self: OptionProtocol[Tuple[U, V]]) -> Tuple[Option[U], Option[V]]:
        ...
        If `self` is [`Some((u, v))`][wraps.primitives.option.Some], this method returns
        ([`Some(u)`][wraps.primitives.option.Some], [`Some(v)`][wraps.primitives.option.Some]) tuple.
        Otherwise, ([`Null`][wraps.primitives.option.Null], [`Null`][wraps.primitives.option.Null]) is returned.
    ...
    def flatten(self: OptionProtocol[OptionProtocol[U]]) -> Option[U]:
        """Flattens an [`Option[Option[T]]`][wraps.primitives.option.Option]
        to [`Option[T]`][wraps.primitives.option.Option].
    ...
    def early(self) -> T:
        ...
        Calls to this method are to be combined with [`@early_option`][wraps.early.decorators.early_option]
...
class Null(OptionProtocol[Never]):
    """The [`Null`][wraps.primitives.option.Null] variant of [`Option[T]`][wraps.primitives.option.Option]."""
...
class Some(OptionProtocol[T]):
    """[`Some[T]`][wraps.primitives.option.Some] variant of [`Option[T]`][wraps.primitives.option.Option]."""
...
"""Optional value, expressed as the union of [`Some[T]`][wraps.primitives.option.Some]
and [`Null`][wraps.primitives.option.Null].
"""

NULL = Null()
"""The instance of [`Null`][wraps.primitives.option.Null]."""


def is_some(option: Option[T]) -> TypeIs[Some[T]]:
    """This is the same as [`Option.is_some`][wraps.primitives.option.OptionProtocol.is_some],
...
def is_null(option: Option[T]) -> TypeIs[Null]:
    """This is the same as [`Option.is_null`][wraps.primitives.option.OptionProtocol.is_null],
    except it works as a *type guard*.
    """
